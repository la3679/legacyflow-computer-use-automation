...
One initial navigation exceeded the 5-second timeout during an earlier local audit with concurrent browser workloads. That test passed in isolation and the subsequent complete serial suite passed. The suite emits one upstream Starlette/AnyIO deprecation warning. These observations are retained rather than represented as application failures or silently omitted.
